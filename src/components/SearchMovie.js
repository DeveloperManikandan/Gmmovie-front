...
const SearchMovie = () => {
      ...
      const submitHandler = async (e) => {
            ...
            try {
                  const res = await axios.get(`https://api.themoviedb.org/3/search/movie?query=${searchMovie}&include_adult=false&language=en-US&page=1`, options,{ timeout: 10000 });
...
{
(searchedMovie && searchedMovie.length > 0)?(<MovieList title={movieName} searchMovie={true} movies={searchedMovie}/>) : (<h1>Not found...</h1>) 
}
</>
      )
}
